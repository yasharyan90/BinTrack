...
const SEALS: {
...
}[] = [
  {
    value: 'intact',
    label: 'Intact',
    hint: 'Number matches, no tampering.',
    icon: ShieldCheck,
    tone: 'data-[active=true]:border-success data-[active=true]:bg-success/15 data-[active=true]:text-success ',
  },
  {
    value: 'broken',
    label: 'Broken',
    hint: 'Raises a discrepancy alert.',
    icon: ShieldAlert,
    tone: 'data-[active=true]:border-destructive data-[active=true]:bg-destructive/15 data-[active=true]:text-destructive ',
  },
  {
    value: 'missing',
    label: 'Missing',
...
export default function GrnNew() {
  ...
  const [form, setForm] = useState({
    ...
    gate_entry_no: '',
    seal_number: '',
    seal_status: 'intact' as SealStatus,
    challan_number: '',
    invoice_number: '',
    shipment_id: '',
    note: '',
  ...
  const canSubmit =
    !!poId && form.vehicle_number.trim().length > 0 && form.driver_name.trim().length > 0
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (form.seal_status !== 'intact') {
        showWarning(
          'Seal discrepancy raised',
          'Admins have been alerted. Photograph the seal before opening.',
        )
      }
    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          ...
          <Card className="border-l-4 border-l-info">
            <CardContent className="space-y-4 p-4 pt-4">
              ...
              <Field
                ...
                htmlFor="po"
                required
                hint={posLoading ? 'Loading open orders…' : undefined}
              >
                <Select value={poId} onValueChange={setPoId}>
                  <SelectTrigger id="po">
                    <SelectValue placeholder="Choose an open purchase order" />
                  </SelectTrigger>
                  <SelectContent>
                    {openPos.map((row) => (
              ...
              {po && (
                <div className="grid gap-3 rounded-lg bg-info/10 p-3 sm:grid-cols-3">
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-small text-muted-foreground">
                      {po.po.expected_date
                        ? `by ${formatDate(po.po.expected_date)}`
                        : 'no date set'}{' '}
                      · <PoStatusChip status={po.po.status} />
                    </p>
                  </div>
                    ...
                    {po.lines.map((l) => (
                      ...
                      >
                        <span className="min-w-0 truncate">
                          <span className="font-mono text-small">{l.sku}</span> · {l.name}
                        </span>
                  ...
                  {po.grns.length > 0 && (
                    <p className="sm:col-span-3 text-small text-muted-foreground">
                      {po.grns.length} earlier {po.grns.length === 1 ? 'delivery' : 'deliveries'}{' '}
                      against this order — partial receiving is fine.
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Gate entry no." htmlFor="gate" hint="Optional.">
                  <Input
                    ...
                    onChange={(e) => set('gate_entry_no', e.target.value)}
                    placeholder="G-0042"
                  />
                </Field>
          ...
          <Card className="border-l-4 border-l-reserved">
            <CardContent className="space-y-4 p-4 pt-4">
              <Step
                n={3}
                title="Shipment verification"
                icon={FileText}
                tone="bg-reserved text-reserved-foreground"
              />
              <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {SEALS.map(({ value, label, icon: Icon, tone }) => (
                      <button
                        ...
                        className={cn(
                          'flex flex-col items-center gap-1 rounded-md border border-border px-2 py-2 text-small transition-colors hover:bg-accent',
                          tone,
                        )}
                ...
                <Field label="Delivery challan number" htmlFor="challan">
                  <Input
                    ...
                    onChange={(e) => set('challan_number', e.target.value)}
                    placeholder="DC-1001"
                    className="font-mono"
                  />
                </Field>
                <Field label="Invoice number" htmlFor="invoice">
                  <Input
                    ...
                    placeholder="INV-2026-88"
                    className="font-mono"
                  />
                </Field>
                <Field
                  label="Shipment / package ID"
                  htmlFor="shipment"
                  hint="If the vendor provides one."
                >
                  <Input
                    id="shipment"
              ...
              {form.seal_status !== 'intact' && (
                <p
                  ...
                  role="status"
                >
                  A {form.seal_status} seal will be recorded as a discrepancy and admins alerted the
                  moment you save. Photograph it on the next screen.
                </p>
              )}
            </CardContent>
          </Card>
        ...
        <div className="space-y-4">
          <Card className="border-l-4 border-l-success">
            <CardContent className="space-y-3 p-4 pt-4">
              ...
              />
              <dl className="space-y-2 rounded-lg bg-success/10 p-3 text-sm">
                ...
                <div className="flex justify-between gap-3">
                  ...
                  <dd className="font-mono text-small">{profile?.id.slice(0, 8)}…</dd>
                </div>
                <div className="flex justify-between gap-3">
              ...
              <p className="text-small text-muted-foreground">
                Recorded from your session — it cannot be edited.
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="space-y-3 p-4 pt-4">
              ...
              <p className="text-small text-muted-foreground">
                A GRN number is generated now. Nothing enters inventory until the counts are
                verified and put away.
              </p>
            </CardContent>
          </Card>
        </div>
